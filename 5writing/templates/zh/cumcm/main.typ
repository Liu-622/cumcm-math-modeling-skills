...
#heading(numbering: none, outlined: false)[附录 A #h(1em) 支撑材料文件列表与完整源程序]
#include(file)
]
#let ai-used = none
#let ai-purpose = [PLACEHOLDER：按当届AI规则和真实用途填写]
#let ai-statement() = {
  heading(numbering: none, outlined: false)[AI工具使用声明]
  if ai-used == true {
    [本参赛队在竞赛过程中使用了AI工具，主要用于#ai-purpose，详细使用情况见支撑材料。]
  } else if ai-used == false {
    [本参赛队在竞赛过程中未使用任何AI工具。]
  } else {
    [PLACEHOLDER：必须按真实情况把 `ai-used` 设置为 `true` 或 `false`。]
  }
}
