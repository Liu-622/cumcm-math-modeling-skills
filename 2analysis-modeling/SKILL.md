...
以题面明确编号的顶层问题为优先划分依据，例如“问题一/二/三”“Problem 1/2/3”。题面没有顶层编号时，按明确任务和交付要求划分并记录依据；不要把背景描述、数据说明误当成独立子问题。
...
如果某个假设会让后续问题的新增条件没有边际效果，要核查题意和约束；允许条件在当前数据下不活跃，不能为了制造差异更改题意。
