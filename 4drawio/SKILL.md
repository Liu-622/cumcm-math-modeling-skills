...
按实际表达需要选择图示，无必要时可在 `reports/DRAWIO_REPORT.md` 中说明本阶段不适用，不设置最低图数。
...
用当前环境的文件编辑工具保存完整 XML，并检查节点 ID 和边引用有效。XML 结构示例：
...
先检测可用的 DrawIO 可执行文件，以下示例按当前 shell 选择。PowerShell：

```powershell
$drawioCommand = Get-Command drawio, draw.io, draw.io.exe -ErrorAction SilentlyContinue | Select-Object -First 1
if ($drawioCommand) {
  & $drawioCommand.Source --export --format pdf --crop --output figures/fig_roadmap.pdf figures/fig_roadmap.drawio
  if ($LASTEXITCODE -ne 0) { throw 'DrawIO PDF export failed' }
}
```

Bash（仅在已存在 Bash 的环境中）：
