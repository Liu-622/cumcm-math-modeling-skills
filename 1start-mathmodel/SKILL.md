...
先读取用户当前请求、已有项目和 `plan.md`，沿用已明确的选择，不重复确认。仅对无法推断且会实质影响结果的选择提问；未指定引擎时优先沿用可编译的现有项目，否则选本机可用引擎（两者均可用则默认 LaTeX），并记录默认值。使用当前客户端实际可用的交互方式，不依赖特定问答工具。
...
3. **论文语言**：按当届比赛要求和交付用途确定；MCM/ICM 正式提交版需核验英文要求，用户要求的中文学习稿需标明用途。
...
- `5writing` 负责决定图表在论文中的位置，并按所选引擎写入图表代码：
  ...
  - LaTeX：从 `paper/` 编译时，正文与章节内均用 `\includegraphics[width=0.85\textwidth]{../figures/xxx.pdf}`；图片路径按编译工作目录及 `\graphicspath` 解析。
