...
本 skill 的 `cumcm-c-problem/scripts/init_project.py` 只初始化独立 C 题 LaTeX 骨架，需要另行提供合法来源的 `cumcmthesis.cls`；它不创建总控状态或证据注册表。完整分阶段项目由 `cumcm-c-national-prize-workflow/scripts/init_project.py` 初始化，不能混用两个同名脚本。

1. 获取并完整阅读赛题、附件、数据字典和官方说明。
2. 若允许联网，先检索当年 CUMCM 官网的：参赛规则、论文格式规范、AI 工具规定、支撑材料要求。旧规则只能作参考。
   处理2026论文时同时完整读取 `../_references/cumcm_2026_format_spec.md`；其中标明的PDF条款是格式硬门。
...
硬要求：
- 电子版第 1 页只容纳**标题、摘要、关键词**，页脚中部页码为阿拉伯数字 `1`；问题重述必须从第 2 页开始。
- 关键词数量服从当届有效文件。2026论文格式PDF没有规定关键词数量；3–5个可作写作建议，不能作为格式硬门。
...
本包按用户明确要求将正文固定为25–28页（含参考文献、不含摘要和附录），作为验收硬门，范围外不得通过；禁止凑页。2026格式条款仍单独遵守：摘要专用页1页，正文不设置目录，正文后的附录页数不限。25–28页是用户要求，不冒充官方规定。
...
先完整读取 `../_references/cumcm_2026_format_spec.md`，并优先沿用经核验的 `cumcmthesis` 2026模板或行为等价模板：
- 不自行重设正文字体、字号、页边距、标题字号与编号体系；
- 保留三线表、figure/subfigure、公式编号、交叉引用的模板习惯；
- 电子版使用模板对应的 `withoutpreface` 方式；
- 使用 XeLaTeX；
- 白色A4，四边页边距至少2.5cm；摘要页页码从1开始且页脚居中；
- 不加目录；
- 正文不超过30页；附录列出支撑材料全部文件，并包含全部完整可运行源程序及交互命令；
- 表格优先 `booktabs` 三线表；
- 图题、表题、公式和正文引用统一；
- overview 图与关键结构图需要专门设计，不允许自动布局导致重叠。

当前骨架位于 `assets/template/paper_skeleton.tex`；完整写作优先使用 `5writing` 的 CUMCM 2026模板。细则见 `references/latex-template-guide.md`，历史示例文件不得覆盖2026规范。
...
project/
├── paper/
│   ├── main.tex       # 可编辑源文件，不含身份信息
│   └── paper.pdf      # 电子版单一PDF，≤20MB，首页为摘要页
├── code/              # 全部完整可运行源程序/交互命令
├── data/              # 自主查阅或处理数据；不重复打包赛题原始数据
├── figures/
├── support/           # 必要中间结果、AI说明等支撑材料
├── supporting-materials.zip  # 单一ZIP/RAR，≤20MB；无支撑材料时可不生成
└── README.md          # 复现入口和文件说明，不进入论文正文
```

代码必须与论文结果一致，并能从 README 或统一入口脚本复现关键表和关键图。论文附录还必须逐项列出支撑材料文件，并嵌入全部完整源程序；只在压缩包中保存完整代码不合格。最终提交包不包含 `.aux`、`.log`、`.out`、`.synctex.gz` 等编译中间文件。
