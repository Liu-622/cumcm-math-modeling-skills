...
def main() -> int:
    ...
    cls = Path(args.class_file).resolve() if args.class_file else None
    if cls is not None and not cls.is_file():
        raise SystemExit(f"class file not found: {cls}")
    targets = ["example.tex", "notes/template_reference.tex", ".gitignore", "README.md"]
    if cls is not None:
        targets.append("cumcmthesis.cls")
    existing = [str(out / name) for name in targets if (out / name).exists()]
    if existing:
        raise SystemExit("Refusing to overwrite existing project files: " + ", ".join(existing))
