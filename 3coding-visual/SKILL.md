...
AI 在实现、求解和作图过程中，必须把关键中间过程保存成数据并做好记录，例如清洗后的数据摘要、模型参数、迭代历史、约束检查、灵敏度分析过程、图表所用数据和运行日志。中间数据优先保存到 `results/`，图表文件放在 `figures/`；已有项目可沿用实际目录，并在 `reports/RESULTS_REPORT.md` 中说明文件用途。
...
- 优先输出 PDF 矢量图；图像数据和栅格热图可嵌入 PDF，不把 PDF 扩展名视为全矢量证明。
- 先探测可用中文字体并验证实际渲染无缺字；记录随机种子、软件版本和生成命令。
