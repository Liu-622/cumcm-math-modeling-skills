...
// Enable paragraph justification with a portable baseline.
...
Resolve every bundled path relative to the directory containing this `SKILL.md`. The mirrored documentation retains website links: map `/docs/reference/foo/` to local `docs/reference/foo.md` or `docs/reference/foo/index.md`, and map `/reference/foo/` the same way after adding the `docs/` prefix. Strip URL fragments when locating the file and search the requested heading within it. These are website routes, not Windows drive-root paths. If no matching file is bundled, resolve the route against `https://typst.app` (using `/docs/` for documentation) rather than claiming the local reference exists.

## Detailed instructions

1. **Check documentation and runtime compatibility**. Inspect `typst --version` when the compiler is available. The bundled documentation is a snapshot; verify function names, parameters, and syntax against it, then check that the installed compiler supports the features used. For older runtimes, adapt unsupported optional features rather than assuming the snapshot matches the installed version. In particular, add optional character-level `justification-limits` only after checking support.
...
- POSIX example: `printf '#metadata(1 + 2) <probe>\n' | typst query - "<probe>" --field value --one`
- PowerShell example: `'#metadata(1 + 2) <probe>' | typst query - '<probe>' --field value --one`

### Post-edit formatting checks

1. **Check whether `typstyle` is available** with `Get-Command typstyle -ErrorAction SilentlyContinue` in PowerShell or `command -v typstyle` in a POSIX shell. If it is unavailable, skip the remaining formatting checks.
...
5. **Keep optional formatting within the requested scope**. If the diff reaches untouched pre-existing code, skip those formatter changes, keep the requested edits, and continue compilation. Ask only when broader formatting is necessary to complete the user's request; existing authorization for whole-file formatting still applies.
...
If you see "unknown font family" warnings, inspect `typst fonts` and choose an installed font covering the document's language while preserving any required typeface rules. In Chinese documents, do not assume the default fallback covers CJK characters. Recompile and inspect the rendered text; successful compilation alone does not prove glyph coverage.
