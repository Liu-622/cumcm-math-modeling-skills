...
- Resolve bundled paths relative to this `SKILL.md`, independently of the user's working directory. Retrieve historical cases with `python -X utf8 "<skill-directory>/scripts/search_cases.py" "<problem description>" --top 6`, replacing `<skill-directory>` with the resolved path. Read the returned cards relative to that skill directory; do not load all cards.
